      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
            ...
            Eigen::VectorXd y_way(ptsy.size());
            ...
            for(int i = 0; i < ptsx.size(); i++) {
                ...
                y_way[i] = dx * sin(-psi) + dy * cos(-psi);
            }
            
            auto coeffs = polyfit(x_way, y_way, 3); // Fit waypoints
            const double cte = polyeval(coeffs, 0);
            const double epsi = -atan(coeffs[1]); //-f'(0)
            
            double delta = j[1]["steering_angle"]; // convert to [-1..1] range
          ...
          vector<double> next_y_vals;
            ...
            for(int i = 1; i < n_points; i ++){
                next_x_vals.push_back(i * incre);
                next_y_vals.push_back(polyeval(coeffs, i * incre));
            }
